...
class CalvinDataset(RLBenchDataset):
    ...
    def __init__(
        ...
        # print("instructions为:", instructions)
        ...
        for task, eps in episodes_by_task.items():
            ...
            # eps = eps[:int( 0.05 * len(eps))]
            self._episodes += eps
            self._num_episodes += len(eps)
            # inject cache accelerate trainning time
            # for _, _, file in eps:
            #     self._cache[file] = loader(file)
    ...
    def __getitem__(self, episode_id):
        ...
        if self._instructions is not None:
            ...
            # print("1::", episode[6][1])
            # print("2::", episode[6][1])
            # print("正在处理instruction")
            # import pdb; pdb.set_trace()
            instr = torch.as_tensor(self._instructions[instr_ind])
            instr = instr.repeat(len(rgbs), 1, 1)
            augmented_text = append_reasoning_suffix(self._instructions_text[instr_ind])
            instr_text = [augmented_text for _ in frame_ids]
        ...
        ret_dict = {
            ...
            "instr_text": instr_text,  # a string 这里记得把格式改为(n_frame, string)
